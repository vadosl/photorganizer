...
def search(request):
    ...
    """
    Необходимо переделать функцью search and update_and_filter,так как в первой нерационально
    обрабатываются запросы пост и гет, в результате функция зря считает parameters при get запросе.
    ...
    #TODO при возврате на первую страницу со второй переменной parameters не
    # присваивается значение request.session["parameters"], в результе
    # "назад на первую" дает пустую страницу.
    # хорошо бы реазизовать все эти методы  на CBV
    if page != 1 and "parameters" in request.session:
        parameters = request.session["parameters"] # а как же первая страница ????
    else:
...
def update_and_filter(images, p):
    # TODO
    #  Razbit' funkciyu update_and_filter na dve funkcii,
    #  tak kak funkciya update uje ispol'zuetsya v drugom predstavlenii i s drugogo urla.
    # Proishodit dublirovanie koda
